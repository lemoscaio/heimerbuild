export function ChampionSkills(props: any) {
	...
	function createSkillELement(skillKey: any) {
		return props.abilities[skillKey].map((skill: any) => {
			return (
						...
						<div>
							{skill.cooldown && (
								<>
									<h6>
										Cooldown:{" "}
										{skill.cooldown.modifiers.map((modifier: any) => {
						...
						<div>
							{skill.cost && (
								<>
									<h6>
										Cost: ''
										{skill.cost.modifiers.map((modifier: any) => {
											return <>{modifier.values.join(" / ")}</>
										})}
									</h6>
								</>
